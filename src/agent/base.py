...
class Agent(object):
    ...
    def __init__(self, args, env):
        ...
        if not getattr(self, 'disable_default_actor', False):#Falseはデフォルト値。attrが定義されてればその値を返す
        ...
        self.g_norm = Normalizer(size=args.dim_goal,
                                 default_clip_range=self.args.clip_range)
        self.sampler = Sampler(args, args.reward_func)
    ...
    def collect_rollout(self, uniform_random_action=False, stochastic=True):
        ...
        for i in range(n_episodes):
            ...
            for t in range(T):
                ...
                new_o, r, d,truncated, info = self.env.step(a)#M
    ...
    def eval_agent(self):
        ...
        for _ in range(self.args.eval_rollout_n_episodes):
            ...
            for _ in range(self.args.max_episode_steps):
                ...
                o, _, _,_, info = self.env.step(a)#M
